...
def is_duplicate_page(page1, page2): # for detecting duplicates 
    # Compare the pixel data of two pages
    return page1.tobytes() == page2.tobytes()

def redact_documents(file_list, output_dir): # for redacting documents
    redacted_files = []
    total_time = 0
    ...
    for i, file in enumerate(file_list):
        ...
        matched_word, truth, redacted_filename = process_image(file, i + 1)
        ...
        if len(file_list)>1: 
            progress_text.text(f"Processed {i + 1} out of {len(file_list)} files")
        else:
            progress_text.text(f"Processed {i + 1} out of {len(file_list)} file")

    return redacted_files, success_count

def create_zip_and_download(file_list):
    # Create a temporary directory
    temp_dir = "temp_download"
    if not os.path.exists(temp_dir):
        os.makedirs(temp_dir)

    # Copy the redacted files to the temporary directory
    ...
    # Create a zip file
    zip_filename = f"Masked_files.zip"
    ...
    st.download_button("Download Masked Files", data=open(zip_filename, "rb").read(), file_name=zip_filename, mime="application/zip", use_container_width=True)
...
#------------------------------CSS alteration ends------------------------------
    ...
    input_files = st.file_uploader("Upload TIFF Files", accept_multiple_files=True,help="Click on the Browse button to select files from your system or drag and drop files here. \n\rOnly TIFF files are allowed",type = ['tif','tiff'])
    total_files=0
    ...
    if input_files:
        ...
        if process_button:
            ...
            redacted_files, success_count = redact_documents(file_list, "temp")
            
            create_zip_and_download(redacted_files)
            ...
            with col1:
                if total_files > 0 :
                    st.success("Click on the Download Masked Files button to download the files")
